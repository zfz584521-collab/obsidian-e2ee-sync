...
- Added `npm.cmd run check:commercial-sts` for credential-free health and readiness checks with strictly whitelisted output.
...
```text
npm.cmd test: 25 test files, 175 tests passed
...
```text
release\obsidian-sync-plugin-0.1.1-commercial-sts-20260723-140127.zip
```
...
- The public production health endpoint currently returns `ok`, while `/readyz` returns HTTP `404`; deploy the current backend before treating the production instance as ready.
